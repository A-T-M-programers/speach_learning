...
import 'package:speach_learning/core/network/get_token.dart';
...
class ServicesLocator {
  void init(){
    ...
    sl.registerLazySingleton(() => PhraseBloc(sl(),sl()));
    sl.registerFactory(() => LogInBloc(sl(),sl()));
    sl.registerFactory(() => ReadBloc(sl()));
    sl.registerFactory(() => RoutePageBloc());
    ...
    sl.registerLazySingleton(() => GetToken(sl(), sl()));
    /// Level
  ...
  void initParticipants() async {
    ...
    if(await CheckConnection.getCheckConnectionNetWork()){
      await sl<GetToken>().call().whenComplete(() {
        /// init Options
        BaseOptions option = BaseOptions(
          headers: {HttpHeaders.authorizationHeader: "Bearer " + ApiConstance.token},
