...
abstract class BaseWordRemoteDataSource<T,Parameter> {
  Future<T> call(Parameter parameter);
}

class SetWordStateRemoteDataSource extends BaseWordRemoteDataSource<bool,SetWordStateEvent>{

  SetWordStateRemoteDataSource();

  @override
  Future<bool> call(SetWordStateEvent parameter) async {
    if(await CheckConnection.getCheckConnectionNetWork()) {
      try {
        ...
        final formData = FormData.fromMap({
          "phrase_word_id": parameter.idWord,
          "status":parameter.state,
        });
        final response = await sl<Dio>(instanceName: "Dio").post("${ApiConstance.setWordState}/${parameter.idParticipant}", data: formData);
