...
class SimpleSwitchCustome extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ProfileParticipantBloc, ParticipantState>(
      ...
      print(previos.requestState);
      return previos.requestState != current.requestState;
    }, builder: (context, state) {
      switch (state.requestState) {
        case RequestState.loading:
          return getWidget(false, 1, context);
        case RequestState.loaded:
          print(state.participants!.langApp);
          if (state.participants != null) {
            context.setLocale(Locale(state.participants!.langApp == 1 ? "en" : "ar"));
        ...
        onTap: () {
          if (checkLoaded) {
            if (langApp != 1) {
              ...
              // Participants.setLanApp("en");
              context.read<ProfileParticipantBloc>().add(SetLangParticipantEvent(id: id!, lang: 1));
            } else {
              //In Arabic
              // AlertDialogShow.loadFutureFunction(context,context.setLocale,[Locale("ar")]);
              // Participants.setLanApp("ar");
              context.read<ProfileParticipantBloc>().add(SetLangParticipantEvent(id: id!, lang: 2));
            }
          }
        },
        child: CustomAnimationBuilder<Movie>(
          // control of the animation
          control: langApp == 1 ? Control.playReverse : Control.play,
          // the relative position where animation will start
          startPosition: langApp == 1 ? 1.0 : 0.0,
